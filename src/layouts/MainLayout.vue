...
// Referencia al contenedor principal para resetear el scroll
const contenedorPrincipal = ref(null)

// Vigila cada vez que cambia la URL y resetea el scroll hacia arriba
watch(() => route.path, () => {
...
// Lógica Anti-Trampas: Se oculta en simulador Y en evaluacion oficial
const permitirChat = computed(() => route.name !== 'simulador' && route.name !== 'evaluacion')
...
const enviarMensaje = () => {
  ...
  setTimeout(() => {
    historial.value.push({ 
      rol: 'ia', 
      texto: `Consultando los manuales oficiales sobre "${textoEnviado}"...` 
    })
  }, 1000)
}
</script>
...
<template>
  ...
  <div class="h-[100dvh] bg-slate-50 flex overflow-hidden font-sans relative">
    ...
    <div class="flex-1 flex flex-col min-w-0 relative">
      ...
      <!-- Contenido Principal (Corregido el scroll y agregado el ref) -->
      <main ref="contenedorPrincipal" class="flex-1 overflow-y-auto pt-8 md:pt-10 pb-28 md:pb-10 px-4 sm:px-8 scroll-smooth">
